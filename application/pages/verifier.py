...
from utils.streamlit_utils import displayPDF, hide_sidebar
...
if selected == options[0]:
    ...
    if uploaded_file is not None:
        ...
        with open("rto.pdf", "wb") as file:
            file.write(bytes_data)
        try:
            (uid, candidate_name, license_number, rto_number) = extract_certificate("rto.pdf")
            displayPDF("rto.pdf")
            os.remove("rto.pdf")

            # Calculating hash
            data_to_hash = f"{uid}{candidate_name}{license_number}{rto_number}".encode('utf-8')
