...
import hashlib
from utils.cert_utils import generate_certificate_college
...
options = ("Generate College Certificate","View Past Certificates", "View College Certificates")
...
if selected == options[0]:
    ...
    course_name = form.text_input(label="Degree")
    org_name = form.text_input(label="College Name")

    submit = form.form_submit_button("Submit")
    if submit:
        pdf_file_path = "college.pdf"
        institute_logo_path = "../assets/logo.jpg"
        generate_certificate_college(pdf_file_path, uid, candidate_name, course_name, org_name, institute_logo_path)
        ...
        st.success(f"College Certificate successfully generated with Certificate ID: {certificate_id}")

        # Update the JSON file with the new college certificate information
        update_certificate_json(candidate_name, "College_Certificate_Id", certificate_id)
