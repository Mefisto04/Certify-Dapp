...
options = ("Generate Birth Certificate", "View Birth Certificates")
selected = st.selectbox("", options, label_visibility="hidden")

if selected == options[0]:
    form = st.form("Generate-Certificate")
    uid = form.text_input(label="UID")
    candidate_name = form.text_input(label="Child Name")
    course_name = form.text_input(label="Birth Place and Date")
    org_name = form.text_input(label="Hospital Name")

    submit = form.form_submit_button("Submit")
    if submit:
        pdf_file_path = "birth.pdf"
        ...
        st.success(f"Birth Certificate successfully generated with Certificate ID: {certificate_id}")
