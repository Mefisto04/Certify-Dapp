...
from utils.cert_utils import generate_certificate_rto
...
options = ("Generate RTO Certificate","View Past Certificates", "View RTO Certificates")
...
if selected == options[0]:
    ...
    course_name = form.text_input(label="License Number")
    org_name = form.text_input(label="RTO Number")

    submit = form.form_submit_button("Submit")
    if submit:
        pdf_file_path = "rto.pdf"
        institute_logo_path = "../assets/logo.jpg"
        generate_certificate_rto(pdf_file_path, uid, candidate_name, course_name, org_name, institute_logo_path)
        ...
        st.success(f"RTO Certificate successfully generated with Certificate ID: {certificate_id}")

        # Update the JSON file with the new RTO certificate information
        update_certificate_json(candidate_name, "RTO_Certificate_Id", certificate_id)
